...
Client.prototype.connect = function (port, host) {
	...
	socket.on('error', reconnector); // XXX close is always emitted after error, will always do two connects
};
...
Client.prototype.autoReconnect = function (min, max) {
	...
	min || (min = 100);
        max || (max = 1000000000); // XXX 277 hours? should probably cap at a
        // few minutes, depending on why connection failure is ocurring
