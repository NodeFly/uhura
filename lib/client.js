...
var pfx = fs.readFileSync(__dirname + '/../scripts/client.p12');
...
/**
 * Connect the client to a TLS server
 * 
...
Client.prototype.connect = function (port, host) {
	...
	var socket = this.socket = tls.connect(options, function () {
		if (socket.authorizationError) {
			return socket.emit('error', socket.authorizationError);
		}
		ev.syncSession();
		ev._send(['start', {
			objectMode: true
